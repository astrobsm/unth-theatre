/**
 * Runs the theatre supply-chain domain tests.
 *
 *     npm run test:stock
 *
 * Same harness as the imprest suites: the sources are ordinary vitest files,
 * and this provides the small slice of the vitest API they use so they can run
 * against src/lib/stock without adding a test framework to the project.
 *
 * These cover the part where a mistake is expensive — the quantity arithmetic
 * a store's figures depend on, the refusals that keep expired or controlled
 * stock off a trolley, and FEFO allocation.
 */
const fs = require('fs');
...
const LIB = path.join(ROOT, 'src/lib/stock');
...
console.log(`Theatre supply-chain domain tests (${SUITES.length} suites, run against src/lib/stock)\n`);

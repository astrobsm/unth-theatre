...
echo
echo "${B}Allowing $APP_USER to restart $UNIT${N}"
...
$SYSTEMCTL list-unit-files "$UNIT.service" >/dev/null 2>&1 \
  || warn "$UNIT.service is not a known unit — installing the rule anyway, but check the name"

TMP="$(mktemp)"
trap 'rm -f "$TMP"' EXIT

cat > "$TMP" <<EOF
# Installed by scripts/local-server/install-worker-restart-sudo.sh
#
# Lets the ORM deploy restart the sync worker unattended. Deliberately ONE
# command with fixed arguments: not systemctl generally, not restart of any
# unit. See the script for why the alternatives are worse.
$APP_USER ALL=(root) NOPASSWD: $SYSTEMCTL restart $UNIT
EOF
...
if su - "$APP_USER" -c "sudo -n $SYSTEMCTL restart $UNIT" >/dev/null 2>&1; then
  ok "$APP_USER restarted $UNIT without a password — the deploy can now do this itself"
  echo
  echo "  worker is now active since: $($SYSTEMCTL show "$UNIT" -p ActiveEnterTimestamp --value)"
else
  die "the rule installed but $APP_USER still cannot restart $UNIT.
        Check that $UNIT is the right unit name and that no later file in
        /etc/sudoers.d overrides this one."
fi

/* Standalone generator for the ORM go-live announcement PDF.
 * Writes to public/announcements/UNTH-ORM-GoLive-8June2026.pdf so the
 * file is served as a static asset by Vercel CDN. Run with:
 *   node scripts/generate-announcement-pdf.js
 */
const fs = require('fs');
...
function build() {
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text(
      'UNTH ORM Go-Live Announcement - Issued 5 June 2026 - Page ' +
        i +
        ' of ' +
        totalPages,
      margin,
...
const out = path.join(__dirname, '..', 'public', 'announcements', 'UNTH-ORM-GoLive-8June2026.pdf');
fs.mkdirSync(path.dirname(out), { recursive: true });
const buf = build();
fs.writeFileSync(out, buf);
console.log('Wrote', out, 'bytes=' + buf.length, 'magic=' + buf.slice(0, 4).toString('hex'));

/**
 * Runs the theatre billing domain tests.
 *
 *     npm run test:billing
 *
 * The two things here that must not be wrong: an effective-dated price, so a
 * bill still reprices to what it charged; and a revenue split whose shares sum
 * back to the invoice total exactly, because a kobo lost per invoice is a
...
const LIB = path.join(ROOT, 'src/lib/billing');
